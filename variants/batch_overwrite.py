...
def main():
    ...
    if not target_date:
        print("No target date set. Run from CLI: batch_overwrite.py YYYY-MM-DD [options]")
        return

    target_folder = base_dir / target_date
    if not target_folder.is_dir():
        print(f"Target date folder not found: {target_folder}")
        return

    date_folders = [target_date]
    filter_desc = []
    if FILTER_CAMERAS:
    ...
    print(f"OVERWRITE MODE: processing {target_date}{filter_str} (will re-render and overwrite existing post_noncropped output)")
    ...
    for date_folder in sorted(date_folders, reverse=True):
        ...
        for raw_file in raw_files:
            ...
            # OVERWRITE MODE: do not skip files that already have post_noncropped output — re-render and let rsync overwrite in place
            ...
            num_match = re.match(r'^[LMR]\d{8}_(\d{4})\.MP4$', filename, re.IGNORECASE)
            if num_match:
                ...
                # Numeric filter set but filename doesn't match the expected pattern — skip
                continue
    ...
    # OVERWRITE MODE: skip the "need at least 2 files" gate — process whatever is found
    ...
    # OVERWRITE MODE: process all files for the given date in one run.
    # DaVinci is still restarted every 50 files inside the loop (see end of loop) to avoid memory leak.
    print(f"Processing all {len(files_to_process)} files for {target_date} in one pass")
    ...
    for raw_file, date_folder, post_dir in files_to_process:
        ...
        # OVERWRITE MODE: do NOT skip if expected_output already exists — always re-render.
        # rsync -av used later will atomically overwrite the destination in place.
        ...
        if rendered_file and rendered_file.exists():
            ...
            # OVERWRITE MODE: always rsync; rsync -av overwrites dest in place (no pre-delete on rclone)
            try:
                rsync_copy(rendered_file, dest_render)
                minimize_davinci()
                os.remove(rendered_file)
                print(f"Successfully moved {rendered_file.name} to post directory as {clean_name} (overwrote existing)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Re-render DaVinci post_noncropped output for a specific date, overwriting existing files in place.")
    ...
    try:
        ...
        print(f"Starting batch_overwrite for {FILTER_TARGET_DATE} at {current_time.strftime('%Y-%m-%d %H:%M:%S')}")
        main()
        print(f"batch_overwrite completed at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        print("Forcefully terminating DaVinci Resolve to ensure clean state...")
        os.system("pkill -9 -f 'DaVinci Resolve'")

    except Exception as e:
        print(f"ERROR: batch_overwrite crashed at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
